        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
           {Womendata.map((Womendata) => (
            ...
            >
              ...
              <Link
                to={`/product-details?id=${Womendata.id}`}
                state={{ product: Womendata }}
                className="block"
              >
                {/* Product Image */}
                <img
                  alt={Womendata.imageAlt}
                  src={Womendata.imageSrc}
                ...
                <div className="mt-4 flex justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{Womendata.href}</p>
                    <p className="mt-1 text-sm text-gray-500">
                      {Womendata.color} | {Womendata.category}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-gray-900">₹{Womendata.price}</p>
              ...
              {/* Add to Bag Button */}
              <button
                onClick={() => console.log("Added to Bag:", Womendata.name)}
